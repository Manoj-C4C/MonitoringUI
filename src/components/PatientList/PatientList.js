...
import patientList from '../../redux/patientList/patientListActionCreator.js';

const mapStateToProps = (state) =>{
  return {
    patientsList: state.patientList.patientsList,
    patientsCount: state.patientList.patientsCount
  }
}

const mapDispatchToProps = (dispatch) =>{
  return {
    storePatientList:(response)=> dispatch(patientList(response))
  }
}
...
class PatientList extends Component {
  ...
  componentDidMount() {
    ...
    .then(responseJson => {
        ...
        if(responseJson.docs) {
          this.props.storePatientList(responseJson.docs);
          const possibleCount = this.props.patientsList.filter(value => value.healthstatus === 'possible').length;
  ...
  checkSOS = (possibleCount) => {
    const sosCount = this.props.patientsList.filter(value => (value.morbidity !== 'none' && value.healthstatus === 'positive')).length;
   ...
   */
  filterPatientList() {
    const { userType, filterTextValue } = this.state;
    let filteredPatientsList = this.props.patientsList;
    if(filterTextValue !== '') {
      const filterkey = userType === 1 ? 'name' : '_id';
      filteredPatientsList = this.props.patientsList.filter(x => x[filterkey].toLowerCase().includes(filterTextValue.toLowerCase()));
    }     
  ...
  render() {
    ...
    const { userType, isSearchBoxOpen, dataLoader, filterTextValue } = this.state;
        ...
        !this.state.patientdetail ? (<div>
          <div className="bx--row row-margin">
            <div className="col-lg-6 patient-count">
              {userStatus === 'All' ? 
                <p className="header-title">Showing All {this.props.patientsCount} patients</p> : 
                <p className="header-title">Showing {this.props.patientsCount} COVID-19 {userStatus} patients</p>
              }
            </div>
