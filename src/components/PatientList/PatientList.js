...
import { Tile, Dropdown, Search, Loading } from "carbon-components-react";
import {
  Location16,
  Warning16,
  Search24,
...
class PatientList extends Component {
  ...
  componentDidMount() {
    ...
    if (localStorage.getItem("user_details")) {
      ...
      if (user_details.usertype === "doctor") {
        endpoint =
          this.props.userStatus === "All"
            ? "doctors/103/" + user_details.id
            : this.props.userStatus === "Positive"
            ? "doctors/101/" + user_details.id
            : "doctors/102/" + user_details.id;
      } else {
        endpoint =
          this.props.userStatus === "All"
            ? "patients/status/103"
    ...
    return getapi(endpoint).then(responseJson => {
      ...
      if (responseJson.docs) {
        const possibleCount = responseJson.docs.filter(
          value => value.healthstatus === "possible"
        ).length;
        this.setState({
          patientsList: responseJson.docs,
          patientsCount: responseJson.docs.length
        });
  ...
  checkSOS = possibleCount => {
    const sosCount = this.state.patientsList.filter(
      value => value.morbidity !== "none" && value.healthstatus === "positive"
    ).length;
  ...
  getLastUpdatedInfo(value) {
    if (value.currentAssign.toLowerCase() === "operator") {
      if (
        value.assignedByDoctor &&
        value.assignedByDoctor.hasOwnProperty("name")
      ) {
        return (
          value.assignedByDoctor.name +
          ", " +
          this.generateTimeFormat(value.assignedByDoctor.timestamp)
        );
      } else {
      ...
      if (
        value.assignedByOperator &&
        value.assignedByOperator.hasOwnProperty("name")
      ) {
        return (
          value.assignedByOperator.name +
          ", " +
          this.generateTimeFormat(value.assignedByOperator.timestamp)
        );
      } else {
        return value.name + ", " + this.generateTimeFormat(value.timestamp);
      }
  ...
  filterPatientList() {
    ...
    if (filterTextValue !== "") {
      ...
      filteredPatientsList = patientsList.filter(x =>
        x[filterkey].toLowerCase().includes(filterTextValue.toLowerCase())
      );
    }
              ...
              <div className="col-lg-6 patient-count">
                {userStatus === "All" ? (
                  <p className="header-title">
                    Showing All {patientsCount} patients
                  </p>
                ) : (
                  <p className="header-title">
                    Showing {patientsCount} COVID-19 {userStatus} patients
                  </p>
                )}
              </div>
              <div
                          ...
                          <div className="name_risk_style">
                            <span
                              className={`name_title ${
                                value.healthstatus === "positive"
                                  ? "red-color"
                                  : value.healthstatus === "possible"
                                  ? "yellow-color"
                                  : "green-color"
                              }`}
                            >
